...
═══════════════════════════════════════════════════════
📊 【reason 欄位格式 - 簡潔專業版（150 字內）】
═══════════════════════════════════════════════════════

**reason 必須包含以下內容（用 | 分隔，總長度控制在 150 字以內）**：

格式：「策略訊號 | 技術指標(3個數值) | 籌碼(主力張數) | 進停損目標價」

範例：
「VCP突破,量比1.8倍 | RSI=58,MACD=0.42,5MA>10MA>20MA | 主力+2850張,外資+1500張 | 進48.5停46.0目標54.0(R:R=1:2)」

⚠️ **重要**：reason 欄位必須簡潔，不要寫成長篇報告，只保留關鍵數據
...
[
  {
    ...
    "profitTWD": 4994,
    "stopLoss": 45.89,
    "reason": "VCP突破,量比1.82倍 | RSI=58.3,MACD=0.42>0.31,5MA(48.8)>10MA(47.5)>20MA(46.2) | 主力+2850張,外資+1500張 | 進48.3停45.9目標53.1(R:R=1:2)"
  }
]
\`\`\`

⚠️ **重要提醒**：
- reason 欄位必須簡潔（150 字內），只包含關鍵數據
- 用 | 分隔四個部分：策略訊號、技術指標、籌碼、交易計畫
- 不要寫成長篇報告，AI 必須精簡表達
`;
};
...
⚠️ **reason 欄位格式（必須簡潔，150 字內）**：
格式：「策略訊號 | 技術指標(3個數值) | 籌碼(主力張數) | 進停損目標價」
範例：「VCP突破,量比1.8倍 | RSI=58,MACD=0.42,5MA>10MA>20MA | 主力+2850張 | 進48.5停46.0目標54.0(R:R=1:2)」

🚨 **重要**：reason 不要寫成長篇報告，只保留關鍵數據，用 | 分隔四個部分。`;
