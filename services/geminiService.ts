...
**reason 欄位規則**：
1. 只描述「選股理由」和「技術分析」，200 字以內
2. ⚠️ **禁止在 reason 中寫進場價、停損價、目標價的具體數字**
3. ⚠️ **交易價位由 JSON 欄位 (entryPoint, stopLoss, exitPoint) 提供，不要在 reason 重複**
4. reason 應專注於：策略訊號、技術指標狀態、籌碼分析、趨勢判斷

範例（正確）：
「VCP 波動率收縮突破，量比 1.8 倍確認。技術面：RSI=58 轉強、MACD 零軸上翻紅、均線多頭排列。籌碼面：主力連 3 日買超共 2850 張，外資同步買超。建議分批進場，跌破 20MA 停損。」

範例（錯誤，禁止這樣寫）：
「進場 45.5 元，停損 43.5 元，目標 50.3 元」← 這些數字應該在 JSON 欄位，不要在 reason 重複！
...
[
  {
    ...
    "profitTWD": 4994,
    "stopLoss": 45.89,
    "reason": "VCP 波動率收縮突破，量比 1.82 倍確認多頭動能。技術面：RSI(14)=58.3 轉強、MACD 柱狀圖連續擴大、均線多頭排列 5MA>10MA>20MA。籌碼面：主力連 3 日買超共 2850 張，外資同步買超 1500 張，籌碼集中度高。建議分批進場，跌破 20MA 出場。"
  }
]
\`\`\`

⚠️ **重要提醒**：
- reason 只寫分析理由（200 字內），不要重複寫進場/停損/目標的數字
- 交易價位由 JSON 欄位提供：entryPoint、stopLoss、exitPoint
- 系統會自動在 UI 顯示交易計畫，不需要在 reason 重複
`;
};
...
⚠️ **reason 欄位規則**：
1. 只寫分析理由（策略訊號、技術指標、籌碼分析），200 字內
2. **禁止**在 reason 中重複寫進場價、停損價、目標價的數字
3. 交易價位由 JSON 欄位 (entryPoint, stopLoss, exitPoint) 提供

範例：「VCP 波動率收縮突破，量比 1.8 倍。技術面：RSI=58 轉強、MACD 零軸上翻紅、均線多頭排列。籌碼面：主力連 3 日買超共 2850 張。」`;
