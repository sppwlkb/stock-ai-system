...
【消息面利多】（100-150 字）
- 🔥 產業題材：是否屬於熱門概念股（AI/半導體/電動車/機器人/低軌衛星）
- 公司新聞：法說會利多、大單進帳、新產品發表、併購消息
- 外資評等：是否有券商調升目標價或評等
- 財報/營收：最近營收表現（月增/年增比較），是否創新高
- 估值評估：本益比、股價淨值比是否仍有上升空間

【演算法訊號】（120-180 字）
- 🎯 型態辨識：VCP 波動率收縮 / 杯柄型態 / 旗形整理 / 箱型突破
- 回檔分析：從高點回檔幅度（例：高點 55 元回檔至 48 元，回檔 12.7%）
- 整理天數：盤整區間天數（例：在 48-52 元區間整理 15 個交易日）
- 振幅收斂：整理期間振幅是否逐漸縮小（例：從 8% 收斂至 3%）
- 突破確認：突破日成交量 vs 20 日均量的倍數（例：量比 2.3 倍）

【技術面共振】（150-200 字）
- 📈 RSI(14)：數值（如 62.5）與解讀（突破 50 中線轉強 / 超買 / 超賣）
- 📈 MACD(12,26,9)：DIF、DEA、柱狀圖數值，是否黃金交叉
- 📈 KD(9,3,3)：K 值、D 值，是否黃金交叉
- 📈 布林通道：中軌、上軌數值，股價相對位置
- 📈 均線排列：5MA/10MA/20MA/60MA 具體數值，是否多頭排列
- 📈 量能分析：當日成交量 vs 5 日均量 vs 20 日均量

【籌碼與量價】（120-180 字）
- 🏦 主力動向：近 5 日主力累計買賣超張數（含具體日期區間）
- 🏦 外資動態：近 5 日外資買賣超張數，是否連續買超
- 🏦 投信動態：投信持股變化，是否積極布局
- 🏦 融資融券：融資餘額變化，是否有籌碼洗清跡象
- 📊 關鍵 K 線：標示爆量長紅或縮量回檔的日期與價位
- 📊 支撐壓力：近期關鍵支撐位與壓力位

【資金控管建議】（80-120 字）
- 💰 建議投入比例：佔總資金的 XX%
- 💰 分批策略：分 2-3 批進場，首批 XX%，確認突破後加碼 XX%
- 💰 加碼條件：突破 XX 元且量能維持 1.5 倍以上可加碼
- 💰 減碼條件：跌破 XX 元應減碼 50%
- ⚠️ 風險報酬比：確保至少 1:3 以上
...
【技術面】搜尋符合以下條件的股票：
- VCP 波動率收縮完成，即將突破
- 股價突破近 20 日高點，量能放大 1.5 倍以上
- RSI(14) > 55，突破中線轉強
- MACD 柱狀圖連續擴大，或出現黃金交叉
- KD(9,3,3) 黃金交叉，K > 50
...
【消息面】優先選擇：
- 熱門題材：AI、半導體、電動車、機器人、低軌衛星、重電、軍工
- 法說會釋出利多消息
- 月營收創新高或年增 > 20%
- 外資券商調升目標價
- 低本益比（PE < 15）但營收成長股
...
📊 **即時數據要求**：
請透過 Google Search 獲取**最新**數據：
1. 🔴 **加權指數**：查詢今日真實點位（目前約 23,000 點上下）
2. 🔴 **台指期夜盤**：查詢最新夜盤漲跌點數（領先指標！）
3. 🔴 **股票報價**：查詢該股票今日真實股價
4. 🔴 **技術指標**：RSI、MACD、KD、均線、布林通道具體數值
5. 🔴 **籌碼資訊**：主力、外資、投信近 5 日買賣超
6. 🔴 **新聞消息**：只搜尋 2025 年 12 月的新聞

⚠️ **reason 欄位必須包含六大專業段落**：
【市場環境分析】【消息面利多】【演算法訊號】【技術面共振】【籌碼與量價】【資金控管建議】

每個段落都要有**具體數據**，總字數約 600-900 字。
🔔 **【市場環境分析】必須包含台指期夜盤的漲跌點數和開盤預估！**
🔔 **【技術面共振】必須包含 RSI、MACD、KD、均線的具體數值！**
🔔 **【籌碼與量價】必須包含主力、外資、投信的具體買賣超張數！**
